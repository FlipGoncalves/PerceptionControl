...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        if angle != 0:
            ...
            betaworld = self.orientation - pi/2 + beta
            tetaF = self.orientation + beta

            self.x = Cx + radius*cos(betaworld)
            self.y = Cy + radius*sin(betaworld)

            self.orientation += beta

        else:
            self.x += length*cos(self.orientation)
            self.y += length*sin(self.orientation)
